...
double Elvas::gaugeQC(const double& arg_gSquared, const double& arg_lambdaAbs, const double& arg_lnQR) {
    ...
    if (x < 1.4) {
        ...
        double x7 = x * x6;
        double x8 = x * x7;
        temp = -0.96686103284373 - 1.76813696868318 * x
                + 0.61593151565841 * x2 + 0.145084271024101 * x3
                - 0.0241469799983579 * x4 + 0.00555917805602827 * x5
                - 0.00145020891759152 * x6 + 0.000402580447036276 * x7
                - 0.000115821925959136 * x8 + 0.5 * log(arg_lambdaAbs)
                + (-0.333333333333333 - 2. * x - x2) * arg_lnQR;
    } else {
        double x9 = x5 * x4;
        double s = 7. + 80. * x + 1024. * x2 + 16384. * x3 + 524288. * x4 - 8388608. * x5;
        temp = -27.0091748854198 + 0.000266011476948977 / x4
                + 0.000288600288600289 / x3 + 0.000595238095238095 / x2 + 0.00317460317460317 / x
                + 1.56519636465016 * x - 0.07988363024944 * x2
                + (-3.54033527491510e-6 / x5 - 0.0000404609745704583 / x4 - 0.00051790047450187 / x3
                - 0.0082864075920299 / x2 - 0.265165042944955 / x
                + 4.24264068711929) * sqrt(x) * asin(s / sqrt(s * s +
                79164837199872 * x9)) + (-6.01666666666667 + 0.5 * x2) * log(x)
                + 1.5 * log(3.14159265358979 * (-98796.7402597403
                + 136316.571428571 * x - 136594.285714286 * x2
                + 92160. * x3 + 7372800. * x4 + 6553600. * x5))
                + 0.5 * log(arg_lambdaAbs) + (-0.333333333333333 - 2. * x - x2) * arg_lnQR;
    }
    return temp;
}
